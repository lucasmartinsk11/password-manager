...
const main = async () => {
 ...
 parser.add_argument("-q", "--query", {type: 'str', nargs: 1, help: "Look up entry by URL or USERNAME", metavar: ("[URL,USERNAME]")});
 ...
 if (args.query) {
  var searchTerm = args.query[0];
  let result
  await dbConnection.select('url',  'user_name', 'secrect_key').from("passwords").where((builder) => {
    builder.where({ url: searchTerm }).orWhere({ user_name: searchTerm });
  })
  .then(data=>{
   result = data
  })
  ...
  if (result.length > 0) {
    ...
    console.log("Could not find record matching the value of '%s'", searchTerm );
  }
 process.exit();
}
